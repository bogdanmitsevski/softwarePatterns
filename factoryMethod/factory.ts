...
// Фабричний метод. Використовуємо тоді, коли потрібно створити багато однотипних об'єктів, які отримують 
...
class DaimlerAG {
    create(type: string, year: string) {
        if (type == 'C200') {
            return new Mercedes(type, '$1000', year);
        }
        if (type == 'E350') {
            return new Mercedes(type, '$10000', year);
        }

        if (type == 'E500') {
            return new Mercedes(type, '$15000', year);
        }
    }
}
